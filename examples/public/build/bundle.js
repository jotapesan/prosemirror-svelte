...
var app = (function () {
    ...
    */
     ...
     * @type {Schema}
     */
    var singleLineSchema = new Schema({
        nodes: {
            doc: { content: "text*" },
            text: { inline: true }
        }
     ...
     * @type {Schema}
     */
    var multiLineSchema = new Schema({
        nodes: {
            doc: {
                content: "paragraph+"
            },
    ...
    */
    ...
    var addKey = function (keyMap, name, combo, command) {
        if (!keyMap[name])
            keyMap[name] = [];
        keyMap[name].push({ combo: combo, command: command });
    };
    var createKeyMapConfiguration = function (schema) {
        var config = {};
        addKey(config, 'undo', "Mod-z", undo);
        addKey(config, "redo", "Shift-Mod-z", redo);
        addKey(config, "undoInputRule", "Backspace", undoInputRule);
        if (!mac$4)
            addKey(config, "redo", "Mod-y", redo);
        addKey(config, "joinUp", "Alt-ArrowUp", joinUp);
        addKey(config, "joinDown", "Alt-ArrowDown", joinDown);
        addKey(config, "lift", "Mod-BracketLeft", lift$1);
        ...
        if (!!schema.nodes.hard_break) {
            ...
            if (mac$4)
                addKey(config, "hardBreak", "Ctrl-Enter", cmd);
        }
        ...
        if (!!schema.nodes.paragraph)
            addKey(config, "setBlockTypeParagraph", "Shift-Ctrl-0", setBlockType$1(schema.nodes.paragraph));
        if (!!schema.nodes.code_block)
            addKey(config, "setBlockTypeCode", "Shift-Ctrl-\\", setBlockType$1(schema.nodes.code_block));
        if (!!schema.nodes.heading)
            for (var i = 1; i <= 6; i++) {
                addKey(config, "setHeading".concat(i), "Shift-Ctrl-".concat(i), setBlockType$1(schema.nodes.heading, { level: i }));
            }
        if (!!schema.nodes.horizontal_rule) {
            addKey(config, "insertHorizontalRuler", "Mod-_", function (state, dispatch) {
    ...
    var getKeyMapFromConfig = function (config) {
        var keys = Object.keys(config);
        var bindings = {};
        keys.forEach(function (key) {
            config[key].forEach(function (entry) {
                bindings[entry.combo] = entry.command;
            });
        });
        return keymap(bindings);
    };
    var richTextKeyMapConfiguration = createKeyMapConfiguration(richTextSchema);
    var richTextKeyMapPlugin = getKeyMapFromConfig(richTextKeyMapConfiguration);
    ...
    var mapFromMarks = function (marks) {
        if (!marks)
            return null;
        var map = {};
        for (var i = 0; i < marks.length; i++) {
    ...
    var getMarksForResolvedPosition = function (resolvedPosition) {
        var marks = resolvedPosition.marks();
     ...
     */
    var getCurrentMarks = function (editorState) {
        var _a = editorState.selection, $head = _a.$head, empty = _a.empty, from = _a.from, to = _a.to;
        var marksInSelection = {};
        var storedMarks = mapFromMarks(editorState.storedMarks);
        var marksAtHead = getMarksForResolvedPosition($head);
        if (!empty) {
            editorState.doc.nodesBetween(from, to, function (node) {
                node.marks.forEach(function (mark) {
                    marksInSelection[mark.type.name] = mark;
                });
            });
        }
        var activeMarks = storedMarks ? storedMarks : Object.assign({}, marksInSelection, marksAtHead);
        return {
            activeMarks: activeMarks,
            marksInSelection: marksInSelection,
            marksAtHead: marksAtHead,
     ...
     */
    var getNodeTypeAtSelectionHead = function (editorState) {
        var $head = editorState.selection.$head;
        var node = $head.node();
        return {
            type: node.type,
            attrs: node.attrs
     ...
     * @return {EditorState}
     */
    var createSingleLineEditor = function (content, plugins) {
        if (content === void 0) { content = ""; }
        if (plugins === void 0) { plugins = []; }
        var doc = content ? singleLineSchema.node("doc", null, [
            singleLineSchema.text(content)
        ]) : undefined;
        var selection = doc ? TextSelection.atEnd(doc) : undefined;
        return EditorState.create({
            schema: singleLineSchema,
            doc: doc,
            selection: selection,
            plugins: __spreadArray(__spreadArray([], plugins, true), corePlugins, true)
        });
    };
    /**
     ...
     * @return {EditorState}
     */
    var createMultiLineEditor = function (content, plugins) {
        if (content === void 0) { content = ""; }
        if (plugins === void 0) { plugins = []; }
        var doc, selection;
        if (content) {
            var paragraphs = content.split('\n');
            doc = multiLineSchema.node("doc", null, paragraphs.map(function (paragraph) {
                return multiLineSchema.node("paragraph", null, paragraph ? [multiLineSchema.text(paragraph)] : undefined);
            }));
        ...
        return EditorState.create({
            ...
            doc: doc,
            selection: selection,
            plugins: __spreadArray(__spreadArray([], corePlugins, true), plugins, true)
        });
    };
    /**
     ...
     * @returns {Document}
     */
    var createDocumentFromHtml = function (schema, html) {
     ...
     * @return {EditorState}
     */
    var createRichTextEditor = function (html, plugins) {
        if (html === void 0) { html = ""; }
        if (plugins === void 0) { plugins = []; }
        var doc, selection;
        if (html) {
        ...
        return EditorState.create({
            ...
            doc: doc,
            selection: selection,
            plugins: __spreadArray(__spreadArray(__spreadArray([], corePlugins, true), richTextPlugins, true), plugins, true)
        });
    };
     ...
     */
    var fromJSON = function (json, schema, plugins) {
        if (schema === void 0) { schema = multiLineSchema; }
        if (plugins === void 0) { plugins = []; }
     ...
     * @returns Object
     */
    var toJSON = function (editorState) {
     ...
     * @returns {string}
     */
    var toHTML = function (editorState) {
        var serializer = DOMSerializer.fromSchema(editorState.schema);
        var fragment = serializer.serializeFragment(editorState.doc.content);
        var node = document.createElement('div');
     ...
     * @return {string}
     */
    var toPlainText = function (editorState) {
        if (editorState.doc.childCount === 0) {
            ...
            var paragraphs = [];
            for (var i = 0; i < editorState.doc.childCount; i++) {
     ...
     * @returns {EditorState}
     */
    var clear = function (editorState) {
        var selection = new AllSelection(editorState.doc);
     ...
     * @returns {EditorState}
     */
    var selectAll$1 = function (editorState) {
        var selection = new AllSelection(editorState.doc);
        var transaction = editorState.tr;
        transaction.setSelection(selection);
        return editorState.apply(transaction);
     ...
     * @returns {EditorState}
     */
    var toggleMark$1 = function (editorState, type, attrs) {
        if (attrs === void 0) { attrs = null; }
        var newEditorState;
        var markType = editorState.schema.marks[type];
        var dispatch = function (tr) { return newEditorState = editorState.apply(tr); };
        if (toggleMark(markType, attrs)(editorState, dispatch)) {
            //@ts-ignore
     ...
     * @returns {EditorState}
     */
    var setBlockType$2 = function (editorState, type, attrs) {
        var newEditorState;
        var nodeType = editorState.schema.nodes[type];
        var dispatch = function (tr) { return newEditorState = editorState.apply(tr); };
        if (setBlockType$1(nodeType, attrs)(editorState, dispatch)) {
            //@ts-ignore
            return newEditorState;
        }
        else {
     ...
     * @returns {EditorState}
     */
    var insertImage = function (editorState, attrs, from, to) {
        if (from === void 0) { from = null; }
        if (to === void 0) { to = null; }
        if (from === null)
            from = editorState.selection.anchor;
        if (to === null)
            to = editorState.selection.head;
        var imageNode = editorState.schema.nodes.image.create(attrs);
        var transaction = editorState.tr;
     ...
     * @returns
     */
    var toggleUnderline = function (editorState, attrs) {
        if (attrs === void 0) { attrs = null; }
        var newEditorState;
        var dispatch = function (tr) { return newEditorState = editorState.apply(tr); };
        var markType = editorState.schema.marks["underline"];
        if (toggleMark(markType, attrs)(editorState, dispatch)) {
            //@ts-ignore
            return newEditorState;
        }
     ...
     */
    var handleUndo = function (editorState) {
        var newEditorState;
        var dispatch = function (tr) { return (newEditorState = editorState.apply(tr)); };
     ...
     * @returns new editor State
     */
    var handleRedo = function (editorState) {
        var newEditorState;
        var dispatch = function (tr) { return (newEditorState = editorState.apply(tr)); };
    ...
    /* ProsemirrorEditor.svelte generated by Svelte v3.55.1 */
    ...
    function create_fragment(ctx) {
    	...
    	const block = {
    		c: function create() {
    			...
    			add_location(div, file, 91, 0, 3547);
    		},
     ...
     */
    	...
    	onMount(() => {
    		...
    		Object.assign(Object.assign({}, editorViewProps), {
    				state: editorState,
    ...
    /* examples/src/components/PlainTextEditor.svelte generated by Svelte v3.55.1 */
    ...
    /* examples/src/components/RichTextEditor_101.svelte generated by Svelte v3.55.1 */
...
<p><b>Ctrl-Shift-1,2,3...</b> will set the block type to heading 1,2,3...</p>`;
    ...
    /* examples/src/components/RichTextEditor_102.svelte generated by Svelte v3.55.1 */
    ...
    function create_if_block(ctx) {
    	...
    	const block = {
    		c: function create() {
    			pre = element("pre");
    			t = text(t_value);
    			add_location(pre, file$3, 135, 2, 4233);
    		},
    		m: function mount(target, anchor) {
    	...
    	dispatch_dev("SvelteRegisterBlock", {
    		...
    		type: "if",
    		source: "(135:0) {#if showEditorState}",
    		ctx
    	});
    ...
    function create_fragment$3(ctx) {
    	...
    	let t21_value = (/*activeMarks*/ ctx[6] && /*activeMarks*/ ctx[6].length
    	? /*activeMarks*/ ctx[6].toString()
    	: 'none') + "";
    	...
    	function prosemirroreditor_focus_binding(value) {
    		/*prosemirroreditor_focus_binding*/ ctx[15](value);
    	}
    	...
    	binding_callbacks.push(() => bind(prosemirroreditor, 'focus', prosemirroreditor_focus_binding));
    	prosemirroreditor.$on("transaction", /*handleTransaction*/ ctx[7]);

    	function select_block_type(ctx, dirty) {
    		if (/*isBold*/ ctx[4]) return create_if_block_2;
    	...
    	function select_block_type_1(ctx, dirty) {
    		if (/*isUnderline*/ ctx[3]) return create_if_block_1;
    		return create_else_block;
    	}

    	let current_block_type_1 = select_block_type_1(ctx);
    	let if_block1 = current_block_type_1(ctx);
    	let if_block2 = /*showEditorState*/ ctx[2] && create_if_block(ctx);
    	...
    	const block = {
    		c: function create() {
    			...
    			button8.textContent = "undo";
    			t16 = space();
    			button9 = element("button");
    			button9.textContent = "redo";
    			t18 = space();
    			div1 = element("div");
    			t19 = text("Additional information about the current editor instance:\n\n  ");
    			ul = element("ul");
    			li0 = element("li");
    			b0 = element("b");
    			...
    			t24 = space();
    			t25 = text(/*activeBlockType*/ ctx[5]);
    			t26 = space();
    			p = element("p");
    			label = element("label");
    			t27 = text("Show serialized editor state\n      ");
    			input = element("input");
    			t28 = space();
    			if (if_block2) if_block2.c();
    			if_block2_anchor = empty();
    			add_location(button0, file$3, 75, 2, 2193);
    			add_location(button1, file$3, 76, 2, 2241);
    			set_style(button2, "margin-left", ".5em");
    			add_location(button2, file$3, 78, 2, 2290);
    			set_style(button3, "margin-left", ".5em");
    			add_location(button3, file$3, 81, 2, 2455);
    			set_style(button4, "margin-left", ".5em");
    			button4.disabled = button4_disabled_value = /*activeBlockType*/ ctx[5] === 'paragraph';
    			add_location(button4, file$3, 85, 2, 2635);
    			button5.disabled = button5_disabled_value = /*activeBlockType*/ ctx[5] === 'heading-1';
    			add_location(button5, file$3, 90, 2, 2827);
    			button6.disabled = button6_disabled_value = /*activeBlockType*/ ctx[5] === 'heading-2';
    			add_location(button6, file$3, 94, 2, 2993);
    			button7.disabled = button7_disabled_value = /*activeBlockType*/ ctx[5] === 'heading-3';
    			add_location(button7, file$3, 98, 2, 3159);
    			add_location(button8, file$3, 102, 2, 3325);
    			add_location(button9, file$3, 105, 10, 3454);
    			attr_dev(div0, "class", "controls");
    			add_location(div0, file$3, 74, 0, 2168);
    			add_location(b0, file$3, 117, 8, 3766);
    			add_location(li0, file$3, 117, 4, 3762);
    			add_location(b1, file$3, 121, 6, 3995);
    			add_location(li1, file$3, 120, 4, 3984);
    			add_location(ul, file$3, 114, 2, 3668);
    			attr_dev(input, "type", "checkbox");
    			add_location(input, file$3, 128, 6, 4124);
    			add_location(label, file$3, 127, 4, 4082);
    			add_location(p, file$3, 126, 2, 4074);
    			attr_dev(div1, "class", "controls");
    			add_location(div1, file$3, 110, 0, 3581);
    		},
    		l: function claim(nodes) {
    			throw new Error("options.hydrate only works if the component was compiled with the `hydratable: true` option");
    		},
    		m: function mount(target, anchor) {
    			...
    			append_dev(button6, t11);
    			append_dev(div0, t12);
    			append_dev(div0, button7);
    			append_dev(button7, t13);
    			append_dev(div0, t14);
    			...
    			if (!mounted) {
    				dispose = [
    					listen_dev(button0, "click", /*clearEditor*/ ctx[8], false, false, false),
    					listen_dev(button1, "click", /*resetEditor*/ ctx[9], false, false, false),
    					listen_dev(button2, "click", /*handleToggleBold*/ ctx[10], false, false, false),
    					listen_dev(button2, "mousedown", preventDefault, false, false, false),
    					listen_dev(button3, "click", /*handleToggleUnderline*/ ctx[11], false, false, false),
    					listen_dev(button3, "mousedown", preventDefault, false, false, false),
    					listen_dev(button4, "click", /*handleSetBlockType*/ ctx[12]('paragraph'), false, false, false),
    					listen_dev(button4, "mousedown", preventDefault, false, false, false),
    					listen_dev(button5, "click", /*handleSetBlockType*/ ctx[12]('heading', { level: 1 }), false, false, false),
    					listen_dev(button5, "mousedown", preventDefault, false, false, false),
    					listen_dev(button6, "click", /*handleSetBlockType*/ ctx[12]('heading', { level: 2 }), false, false, false),
    					listen_dev(button6, "mousedown", preventDefault, false, false, false),
    					listen_dev(button7, "click", /*handleSetBlockType*/ ctx[12]('heading', { level: 3 }), false, false, false),
    					listen_dev(button7, "mousedown", preventDefault, false, false, false),
    					listen_dev(button8, "click", /*click_handler*/ ctx[16], false, false, false),
    					listen_dev(button8, "mousedown", preventDefault, false, false, false),
    					listen_dev(button9, "click", /*click_handler_1*/ ctx[17], false, false, false),
    					listen_dev(button9, "mousedown", preventDefault, false, false, false),
    		...
    		p: function update(ctx, [dirty]) {
    			...
    			if (!current || dirty & /*activeBlockType*/ 32 && button4_disabled_value !== (button4_disabled_value = /*activeBlockType*/ ctx[5] === 'paragraph')) {
    				prop_dev(button4, "disabled", button4_disabled_value);
    			}

    			if (!current || dirty & /*activeBlockType*/ 32 && button5_disabled_value !== (button5_disabled_value = /*activeBlockType*/ ctx[5] === 'heading-1')) {
    				prop_dev(button5, "disabled", button5_disabled_value);
    			}

    			if (!current || dirty & /*activeBlockType*/ 32 && button6_disabled_value !== (button6_disabled_value = /*activeBlockType*/ ctx[5] === 'heading-2')) {
    				prop_dev(button6, "disabled", button6_disabled_value);
    			}

    			if (!current || dirty & /*activeBlockType*/ 32 && button7_disabled_value !== (button7_disabled_value = /*activeBlockType*/ ctx[5] === 'heading-3')) {
    				prop_dev(button7, "disabled", button7_disabled_value);
    			}

    			if ((!current || dirty & /*activeMarks*/ 64) && t21_value !== (t21_value = (/*activeMarks*/ ctx[6] && /*activeMarks*/ ctx[6].length
    			? /*activeMarks*/ ctx[6].toString()
    			: 'none') + "")) set_data_dev(t21, t21_value);

    			if (!current || dirty & /*activeBlockType*/ 32) set_data_dev(t25, /*activeBlockType*/ ctx[5]);
    ...
    function instance$3($$self, $$props, $$invalidate) {
    	...
    	function handleToggleUnderline(event) {
    		$$invalidate(0, editorState = toggleUnderline(editorState));
    	}

    	function handleSetBlockType(type, attrs = null) {
    		return function (event) {
    			$$invalidate(0, editorState = setBlockType$2(editorState, type, attrs));
    		};
    	}
    	...
    	const click_handler = e => $$invalidate(0, editorState = handleUndo(editorState));
    	const click_handler_1 = e => $$invalidate(0, editorState = handleRedo(editorState));
    	...
    	$$self.$inject_state = $$props => {
    		...
    		if ('editorState' in $$props) $$invalidate(0, editorState = $$props.editorState);
    		if ('currentMarks' in $$props) $$invalidate(13, currentMarks = $$props.currentMarks);
    		if ('isUnderline' in $$props) $$invalidate(3, isUnderline = $$props.isUnderline);
    		if ('isBold' in $$props) $$invalidate(4, isBold = $$props.isBold);
    		if ('nodeAtSelectionHead' in $$props) $$invalidate(14, nodeAtSelectionHead = $$props.nodeAtSelectionHead);
    		if ('activeBlockType' in $$props) $$invalidate(5, activeBlockType = $$props.activeBlockType);
    		if ('activeMarks' in $$props) $$invalidate(6, activeMarks = $$props.activeMarks);
    	};
    	...
    	$$self.$$.update = () => {
    		if ($$self.$$.dirty & /*editorState*/ 1) {
    			 $$invalidate(13, currentMarks = editorState ? getCurrentMarks(editorState) : null);
    		}

    		if ($$self.$$.dirty & /*currentMarks*/ 8192) {
    			 $$invalidate(6, activeMarks = currentMarks
    			? Object.keys(currentMarks.activeMarks)
    			: []);
    		}

    		if ($$self.$$.dirty & /*editorState*/ 1) {
    			 $$invalidate(14, nodeAtSelectionHead = editorState
    			? getNodeTypeAtSelectionHead(editorState)
    			: {});
    		}

    		if ($$self.$$.dirty & /*nodeAtSelectionHead*/ 16384) {
    			 $$invalidate(5, activeBlockType = getBlockType(nodeAtSelectionHead));
    		}

    		if ($$self.$$.dirty & /*currentMarks*/ 8192) {
    			 $$invalidate(4, isBold = currentMarks && currentMarks.activeMarks && currentMarks.activeMarks.strong);
    		}

    		if ($$self.$$.dirty & /*currentMarks*/ 8192) {
    			 $$invalidate(3, isUnderline = currentMarks && currentMarks.activeMarks && currentMarks.activeMarks.underline);
    		}
    	};
    ...
    /* examples/src/components/RichTextEditor_103.svelte generated by Svelte v3.55.1 */
    ...
    /* examples/src/components/RichTextEditor_104.svelte generated by Svelte v3.55.1 */
    ...
    function create_fragment$5(ctx) {
    	...
    	let pre;
    	let t7;
    	let t8_value = (/*pasteData*/ ctx[1] || 'Nothing pasted yet') + "";
    	let t8;
    	let t9;
    	let t10;
    	let p;
    	...
    	const block = {
    		c: function create() {
    			...
    			t10 = space();
    			p = element("p");
    			t11 = text("Total pastes: ");
    			t12 = text(/*numberOfPastes*/ ctx[2]);
    		...
    		p: function update(ctx, [dirty]) {
    			...
    			prosemirroreditor.$set(prosemirroreditor_changes);
    			if ((!current || dirty & /*pasteData*/ 2) && t8_value !== (t8_value = (/*pasteData*/ ctx[1] || 'Nothing pasted yet') + "")) set_data_dev(t8, t8_value);
    			if (!current || dirty & /*numberOfPastes*/ 4) set_data_dev(t12, /*numberOfPastes*/ ctx[2]);
    		},
    		i: function intro(local) {
    			if (current) return;
    			transition_in(prosemirroreditor.$$.fragment, local);
    ...
    /* examples/src/components/RichTextEditor_105.svelte generated by Svelte v3.55.1 */
    ...
    /* examples/src/components/ProsemirrorLogo.svelte generated by Svelte v3.55.1 */
    ...
    /* examples/src/components/SvelteLogo.svelte generated by Svelte v3.55.1 */
    ...
    /* examples/src/App.svelte generated by Svelte v3.55.1 */
    ...
    // (68:10) {#each Object.keys(examples) as example (example)}
    function create_each_block(key_1, ctx) {
    	...
    	const block = {
    		...
    		c: function create() {
    			...
    			add_location(option, file$9, 68, 12, 2494);
    			this.first = option;
    		},
    		m: function mount(target, anchor) {
    	...
    	dispatch_dev("SvelteRegisterBlock", {
    		...
    		type: "each",
    		source: "(68:10) {#each Object.keys(examples) as example (example)}",
    		ctx
    	});
